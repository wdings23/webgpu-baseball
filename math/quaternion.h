...
struct quaternion
{
    ...
    {
        bool bDiffX = (fabs(x - q.x) < 0.000001f);
        bool bDiffY = (fabs(y - q.y) < 0.000001f);
        bool bDiffZ = (fabs(z - q.z) < 0.000001f);
        bool bDiffW = (fabs(w - q.w) < 0.000001f);
    ...
    static quaternion slerp(quaternion const& q0, quaternion const& q1, float t);
